...
{
#define _CLAMP_255(x) ((x > 255) ? 255 : (x))
	int			samples;
	...
	if(lightMap)
	{
		...
		m_data = calloc(c, sizeof(unsigned));
		if(m_data)
		{
			m_scan = (byte *)m_data;
			for (i = 0; i < c; i++)
			{
				// 纹理数据的每个成分为4个字节
				// glTexImage2D 成分为GL_UNSIGNED_BYTE, 1个字节
				...
				else
				{
					// GL_RGB
					// 纹理成分解释为RGB
					// 将亮度值的前3字节转换为RGB的各个成分
					m_scan[i * 4] = scan[i * 4];
					m_scan[i * 4 + 1] = scan[i * 4 + 1];
					m_scan[i * 4 + 2] = scan[i * 4 + 2];
				}
				m_scan[i * 4 + 3] = 255;
			}
			data = m_data;
